...
Please report privately through GitHub's [security advisory form](https://github.com/rajanaggarwal11/catalog-doctor/security/advisories/new) rather than opening a public issue.

You should get an acknowledgement within 72 hours and an assessment within a week.

## Scope

`catalog-doctor` reads `package.json`, `pnpm-workspace.yaml`, `tsconfig.json` and source files, and with `--fix` writes back to the first two. It runs no code from the workspace it inspects, spawns no subprocesses, and makes no network requests.
